...
export default function PaymentSuccessScreen() {
  const { plan: planParam, finalAmount: finalAmountParam } =
  ...
  const finalAmount = React.useMemo(() => {
    try {
      return parseFloat(finalAmountParam as string);
    } catch (error) {
      console.error("Error parsing final amount:", error);
      return 0;
    }
  }, [finalAmountParam]);
  ...
  const handleContinue = () => {
    // Navigate to main app or user profile
    router.replace("/(tabs)"); // Navigate to the main tab navigator
  };

  const handleViewPlans = () => {
    router.back(); // Go back to the plans screen
  };
